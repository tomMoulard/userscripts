// ==UserScript==
// @name         Add button to change to daily pull requests
...
(function() {
    'use strict';
    document.getElementById(create_button("Daily pull requests", "#repo-content-pjax-container > div > div.d-flex.flex-justify-between.mb-md-3.flex-column-reverse.flex-md-row.flex-items-end > div.ml-3.d-flex.flex-justify-between.width-full.width-md-auto")).addEventListener("click", (e) => {
        var now = new Date;
        var num_of_days = (now.getUTCDay() === 1) ? 3 : 1;
        javascript:location.href="?q=is%3Apr+is%3Aopen+sort%3Aupdated-desc+-label%3Astatus%2F5-frozen-due-to-age+updated%3A>%3D"+(new Date(Date.now() - 864e5 * num_of_days)).toISOString().slice(0,10) + "T16%3A00%3A00";
    });
})();
